...
class BakeryShop:
    def __init__(self, env, num_employees):
        self.env = env
        self.employees = simpy.PreemptiveResource(env, num_employees)
...
def customer(env, customer_id, bakery_shop):
    ...
    if r < 1 - VIP_customer_prob: # r < 0.95
        priority = 2
    else: # customer is VIP                                       
        priority = 1
                
    with bakery_shop.employees.request(priority=priority, preempt = True) as request:
        try:
            ...
            busy_employees -= 1
            print(f'{customer_id} preempted at {env.now}')
        customers_handled += 1
